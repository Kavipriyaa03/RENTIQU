...
import './AdminBridesmaidProducts.css';

const AdminBridesmaidProducts = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/bridesmaidproducts/bridesmaid')
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8080/api/bridesmaidproducts/bridesmaid/${id}`)
      .then(() => {
  ...
  const handleAddOrUpdateProduct = () => {
    ...
    const request = editingProduct
      ? axios.put(`http://localhost:8080/api/bridesmaidproducts/bridesmaid/${editingProduct.id}`, formData)
      : axios.post('http://localhost:8080/api/bridesmaidproducts/bridesmaid', formData);
  ...
  return (
    <div className="admin-bridesmaid-products-container">
      <h1>Bridesmaid Products Management</h1>
      ...
      <table className="admin-bridesmaid-products-table">
        <thead>
          <tr>
            <th>Image</th>
            <th>Name</th>
          ...
          {products.map(product => (
            <tr key={product.id}>
              ...
              <td>
                ...
                <Link to={`/bridesmaidproduct/${product.id}`}>View</Link>
              </td>
            </tr>
          ))}
        </tbody>
...
export default AdminBridesmaidProducts;
