...
import './AdminGroomsmenProducts.css'; // Update the CSS file path if necessary

const AdminGroomsmenProducts = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/groomsmenproducts/groomsmen') // Update API endpoint
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8080/api/groomsmenproducts/groomsmen/${id}`) // Update API endpoint
      .then(() => {
  ...
  const handleAddOrUpdateProduct = () => {
    ...
    const request = editingProduct
      ? axios.put(`http://localhost:8080/api/groomsmenproducts/groomsmen/${editingProduct.id}`, formData) // Update API endpoint
      : axios.post('http://localhost:8080/api/groomsmenproducts/groomsmen', formData); // Update API endpoint
  ...
  return (
    <div className="admin-groomsmen-products-container"> {/* Update container class name if necessary */}
      <h1>Groomsmen's Products Management</h1>
      ...
      <table className="admin-groomsmen-products-table"> {/* Update table class name if necessary */}
        <thead>
          <tr>
            <th>Image</th>
            <th>Name</th>
          ...
          {products.map(product => (
            <tr key={product.id}>
              ...
              <td>
                ...
                <Link to={`/groomsmenproduct/${product.id}`}>View</Link>
              </td>
            </tr>
          ))}
        </tbody>
...
export default AdminGroomsmenProducts;
