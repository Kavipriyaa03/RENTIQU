...
const FilterSidebar = ({ filters, onFilterChange, isOpen, onClose }) => {
  ...
  return (
    <div className={`filter-sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <h2>By Category</h2>
      {['Saree', 'Lehenga', 'Salwar', 'Anarkali', 'Gown'].map(category => (
        <label key={category}>
          <input 
            type="checkbox" 
            value={category} 
      ...
      <h2>By Price</h2>
      {['1000-2000', '2000-3000', '3000-4000', '4000-50000', '50000-9000'].map(price => (
        <label key={price}>
          <input 
            type="checkbox" 
            value={price} 
      ...
      <h2>By Color</h2>
      {[ 'blue', 'green', 'pink', 'black', 'white', 'orange', 'gold'].map(color => (
        <label key={color}>
          <input 
            type="checkbox" 
            value={color} 
      ...
      <h2>By Occasion</h2>
      {['Wedding', 'Casual', 'Party', 'Festive'].map(occasion => (
        <label key={occasion}>
          <input 
            type="checkbox" 
            value={occasion} 
